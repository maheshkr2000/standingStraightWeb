...
const FreeSurgeriesSection = () => {
  ...
  const [active, setActive] = useState<SectionId>("exist");
  const [selectedSection, setSelectedSection] = useState<SectionId | null>(null);

  const sections = useMemo(
    () => [
      {
        id: "exist" as SectionId,
        label: "Why We Exist",
  ...
  const selectedMeta = selectedSection ? sections.find((item) => item.id === selectedSection) : null;
    ...
    <section className="py-16 md:py-24 bg-slate-50/50">
      <div className="max-w-7xl mx-auto px-6 relative">
        <div className="grid lg:grid-cols-[320px_1fr] gap-12 lg:gap-16 items-start">
          ...
          {/* Mobile summary cards that open details in a modal */}
          <div className="lg:hidden space-y-4">
            {sections.map((item) => (
              <Card
                ...
                className="p-6 bg-white border border-slate-200 shadow-soft hover:shadow-card transition-all duration-300 hover:-translate-y-1 cursor-pointer"
              >
                <p className="text-xs font-bold tracking-[0.18em] uppercase text-medical-teal mb-2">{item.label}</p>
                <h3 className="text-xl font-bold text-slate-900 mb-2">{item.title}</h3>
                <p className="text-sm text-slate-600 mb-3">{item.sub}</p>
                <span className="text-medical-teal font-semibold">Tap to view details</span>
              </Card>
            ))}
          </div>
        ...
        <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-3xl">
          {selectedMeta && (
            <DialogHeader className="space-y-1">
              <DialogTitle className="text-2xl">{selectedMeta.title}</DialogTitle>
          ...
          <div className="space-y-10 pt-2">
            {selectedSection === "exist" && <WhyWeExistContent />}
            {selectedSection === "do" && <WhatWeDoContent />}
            {selectedSection === "takes" && <WhatItTakesContent />}
          </div>
        </DialogContent>
      </Dialog>
    </section>
